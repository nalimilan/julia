...
"""
    validargs(::Type{<:TimeType}, args...) -> Union{Some{ArgumentError}, Null}

Determine whether the given arguments consitute valid inputs for the given type.
Returns a `Union{Some{ArgumentError}, Null}` where [`null`](@ref) signifies success.
"""
function validargs end

"""
    argerror([msg]) -> Union{Some{ArgumentError}, Null}

Construct an `Union{Some{ArgumentError}, Null}` with the given message, or [`null`](@ref) if no message
